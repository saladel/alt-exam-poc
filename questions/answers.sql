...
from 
    checked_out_cart cc
join 
    alt_school.products p on p.id = (cc.event_data ->> 'item_id')::int
group by 1,2
order by 3 desc
limit 1;
...
/*
This query finds the most common location(country) where successful checkouts occurred.
It returns the location and the number of checkouts that occurred in that location.
*/
...
/*
This query identifies customers who abandoned their carts and counts the number of events 
(excluding visits and the checkout event itself i.e failed or cancelled) that occurred before the abandonment. 
...
/*
This query calculates the average number of visits per customer, prioritizing customers who completed a checkout. 
It returns the average_visits rounded to 2 decimal places.
*/
